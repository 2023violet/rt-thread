...
/* -------------------------------------------------------------------------- */
#ifdef _MSC_VER
/* 创建一个名为 "rti_fn$f" 的段，属性只读 */
#pragma section("rti_fn$f", read)
/* 如果打开调试信息，结构体会多带一个 fn_name 成员，否则只有 level 和 fn */
...
/**
 * 假设输入的是INIT_EXPORT(uart_init, "3")
 * 1. 得到一个常量字符串
 * 2. 得到字符串 "uart_init"，方便调试器查看
 * 3. 告诉链接器：把紧接着定义的变量硬塞到 "rti_fn$f" 段
 */
...
struct rt_device
{
    ...
    rt_err_t (*readlink)(rt_device_t dev, char *buf, int len); /**< for dynamic device */
